...
interface PasswordGeneratorProps {
  ...
  onCancel: () => void;
  isEmbedded?: boolean;
  /** Forwarded to the hook — called when the user commits to a generated password. */
  onRecordGenerated?: (password: string) => void;
  /** Notifies the parent of the latest generated value (every regeneration). */
...
export default function PasswordGenerator({
  ...
  const {
    ...
    handleRegenerate,
    handleUsePassword,
  } = usePasswordGenerator({ onUsePassword, enableShortcuts: isEmbedded, onRecordGenerated, onGeneratedChange });

  const generatorContent = (
    <Flex direction="column" gap="3">
      <TextField.Root value={generatedPassword} readOnly size="2" />

      <Box>
        <Flex direction="column" gap="1">
          <span className={fieldLabel}>Length: {length[0]}</span>
          <Slider min={8} max={32} value={length} onValueChange={setLength} />
        </Flex>
      </Box>
